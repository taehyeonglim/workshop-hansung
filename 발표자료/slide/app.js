...
const slides = [
  ...
  },
  {
    ...
    body: `
      <div class="grid grid--3">
        ${card("1. 데이터 지도", "data 폴더의 엑셀 파일이 어떤 주제와 연도를 담고 있는지 표로 정리합니다.", COLORS.teal)}
        ${card("2. 검증된 인사이트", "AI가 만든 분석 결과를 행 수, 합계, 기준연도, 샘플 행으로 다시 확인합니다.", COLORS.red)}
        ${card("3. 웹 슬라이드 초안", "분석 결과를 발표 흐름으로 바꾸고 GitHub Pages로 공유할 수 있는 형태까지 연결합니다.", COLORS.gold)}
      </div>
  ...
  },
  {
    ...
    body: `
      ...
      <div class="grid grid--2" style="margin-top:30px">
        ${card("수강생에게 강조", "처음부터 좋은 질문을 완성하려 하지 않습니다. AI가 낸 첫 결과를 보고 다음 질문을 만드는 것이 핵심입니다.")}
        ${card("강사에게 강조", "막힌 화면은 지나치지 말고 함께 읽습니다. 설치 실패, 파일 판독 실패, 권한 요청은 모두 좋은 학습 장면입니다.", COLORS.blue)}
      </div>
    `,
  ...
  },
  {
    ...
    body: `
      <div class="grid grid--3">
        ${card("파일 보관", "HTML, CSS, JS 파일을 웹에 올릴 수 있는 공간입니다. 오늘 만든 발표자료 파일이 여기에 놓입니다.", COLORS.blue)}
        ${card("변경 기록", "수정할 때마다 어떤 파일이 바뀌었는지 기록됩니다. AI가 만든 결과를 되돌아볼 때 유용합니다.", COLORS.gold)}
        ${card("Pages 배포", "정적 HTML 파일을 별도 서버 없이 웹 주소로 공개할 수 있습니다. 수강생은 링크 하나로 결과물을 확인합니다.", COLORS.teal)}
      </div>
  ...
  },
  {
    ...
    body: `
      <div class="grid grid--4">
        ${card("로그인", "github.com 우측 상단에 내 프로필 아이콘이 보이는지 확인합니다.", COLORS.blue)}
        ${card("이메일 인증", "인증 메일을 눌렀는지 확인합니다. 인증 전에는 일부 기능이 제한될 수 있습니다.", COLORS.gold)}
        ${card("사용자명", "공개 주소에 들어가므로 실명이나 학번을 그대로 쓰지 않도록 안내합니다.", COLORS.red)}
        ${card("결제 화면", "오늘 실습은 무료 계정으로 충분합니다. 유료 플랜 안내가 나오면 건너뜁니다.", COLORS.green)}
      </div>
  ...
  },
  {
    ...
    body: `
      ...
      <div class="grid grid--2" style="margin-top:30px">
        ${card("수강생에게 필요한 이해", "Codex는 채팅창이 아니라 작업 폴더 안에서 파일을 읽고 고치고 확인하는 에이전트입니다.", COLORS.teal)}
        ${card("강사가 지킬 기준", "버튼을 모두 설명하기보다 오늘 쓰는 버튼만 보여줍니다. 프로젝트, 새 스레드, Browser, Terminal, Diff, Approval만 잡으면 충분합니다.", COLORS.gold)}
      </div>
    `,
  ...
  },
  {
    ...
    body: `
      <div class="grid grid--3">
        ${card("설치 확인 스레드", "폴더가 열렸는지, data 폴더가 보이는지, 파일 목록을 읽을 수 있는지 확인합니다.", COLORS.teal)}
        ${card("데이터 분석 스레드", "엑셀 구조 파악, 정제, 차트 데이터 생성, 검증 질문을 진행합니다.", COLORS.blue)}
        ${card("발표자료 제작 스레드", "구성안, HTML/CSS/JS 생성, 화면 검수, GitHub Pages 배포 확인을 맡깁니다.", COLORS.gold)}
      </div>
  ...
  },
  {
    ...
    body: `
      <div class="grid grid--3">
        ${card("무엇을 하려는가", "파일을 읽는지, 파일을 쓰는지, 명령을 실행하는지 먼저 봅니다.", COLORS.blue)}
        ${card("어디까지 접근하는가", "워크숍 폴더 안인지, 폴더 밖인지, 네트워크 접근이 필요한지 확인합니다.", COLORS.red)}
        ${card("왜 필요한가", "분석, 미리보기, 배포처럼 현재 작업과 직접 연결되는 이유가 있는지 묻습니다.", COLORS.gold)}
      </div>
